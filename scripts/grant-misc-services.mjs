 ...
 */
/**
 * Backfill: grant SELECT on misc_services to all existing *_adobe_com read-only users.
 * (misc_services was added to add-user.mjs's TABLES list after these users were created,
 * so they never received the grant.)
 *
 * Usage: node scripts/grant-misc-services.mjs <admin-user> <admin-password> [-x]
...
async function main() {
  ...
  if (!adminUser || !adminPassword) {
    console.error('Usage: node scripts/grant-misc-services.mjs <admin-user> <admin-password> [-x]');
    process.exit(1);
  }
