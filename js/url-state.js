...
function addBasicParams(params) {
  ...
  if (state.hostFilter) {
    params.set('host', state.hostFilter);
  }
  if (state.topN !== DEFAULT_TOP_N) {
    params.set('n', state.topN);
  }
  if (state.showLogs) {
  ...
  if (state.contentTypeMode !== 'count') {
    params.set('ctm', state.contentTypeMode);
  }
}
...
/**
 * Add extra URL params (e.g., domain/domainkey for RUM pages) to URL params.
 * These are set by data-source-specific entry points and preserved across URL updates.
 */
