 ...
 * Shared between loadInpTimeSeries and loadInpBreakdowns
...
/**
 * Fetch RUM data and render the INP time series chart.
 * Uses viewType='inp' for INP-specific classification (good/ni/poor)
 * and applies interpolation for sparse CWV measurement buckets.
 * @param {object} requestContext - From startRequestContext()
 */
async function loadInpTimeSeries(requestContext) {
  const { requestId, scope } = requestContext;
  const isCurrent = () => isRequestCurrent(requestId, scope);

  // Invalidate cache so concurrent loadInpBreakdowns fetches fresh data
  cachedRumResult = null;
  ...
  try {
    ...
    // Cache result for loadInpBreakdowns (if it hasn't fetched yet)
    cachedRumResult = result;

    // Apply interpolation to fill gaps where INP measurements are sparse
...
/**
 * Render breakdown facet tables from RUM INP data.
 * Uses the full facet set for comprehensive INP analysis.
 * @param {object} requestContext - From startRequestContext()
 */
async function loadInpBreakdowns(requestContext) {
  const { requestId, scope } = requestContext;
  const isCurrent = () => isRequestCurrent(requestId, scope);
...
function setRumUrlState(creds) {
  ...
  renderRumNav(document.getElementById('rumNav'), 'inp');
}
...
initDashboard({
  ...
  loadTimeSeries: loadInpTimeSeries,
  loadBreakdowns: loadInpBreakdowns,
});
