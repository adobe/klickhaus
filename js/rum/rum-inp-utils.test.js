...
import {
  INP_BREAKDOWNS,
  interpolateCwvGaps,
} from './rum-inp-utils.js';

describe('rum-inp-utils', () => {
  describe('INP_BREAKDOWNS', () => {
    it('defines all 16 facets from the full facet set', () => {
      assert.lengthOf(INP_BREAKDOWNS, 16);
    });

    it('includes URL, Device Type/OS, and Checkpoint as first three', () => {
      assert.strictEqual(INP_BREAKDOWNS[0].facetName, 'url');
      assert.strictEqual(INP_BREAKDOWNS[1].facetName, 'userAgent');
      assert.strictEqual(INP_BREAKDOWNS[2].facetName, 'checkpoint');
    });

    it('includes External Referrer', () => {
      const facet = INP_BREAKDOWNS.find((b) => b.facetName === 'enterSource');
      assert.isNotNull(facet);
      assert.strictEqual(facet.id, 'breakdown-enterSource');
    });

    it('includes Click targets', () => {
      const facet = INP_BREAKDOWNS.find((b) => b.facetName === 'clickTarget');
      assert.isNotNull(facet);
      assert.strictEqual(facet.id, 'breakdown-clickTarget');
    });

    it('includes Viewmedia', () => {
      const facet = INP_BREAKDOWNS.find((b) => b.facetName === 'mediaTarget');
      assert.isNotNull(facet);
    });

    it('includes Viewblock', () => {
      const facet = INP_BREAKDOWNS.find((b) => b.facetName === 'viewblock');
      assert.isNotNull(facet);
    });

    it('includes Navigate', () => {
      const facet = INP_BREAKDOWNS.find((b) => b.facetName === 'navigate');
      assert.isNotNull(facet);
    });

    it('includes Language', () => {
      const facet = INP_BREAKDOWNS.find((b) => b.facetName === 'language');
      assert.isNotNull(facet);
    });

    it('includes Accessibility', () => {
      const facet = INP_BREAKDOWNS.find((b) => b.facetName === 'accessibility');
      assert.isNotNull(facet);
    });

    it('includes Consent', () => {
      const facet = INP_BREAKDOWNS.find((b) => b.facetName === 'consent');
      assert.isNotNull(facet);
    });

    it('includes Loadresource', () => {
      const facet = INP_BREAKDOWNS.find((b) => b.facetName === 'loadresource');
      assert.isNotNull(facet);
    });

    it('includes Acquisition', () => {
      const facet = INP_BREAKDOWNS.find((b) => b.facetName === 'acquisitionSource');
      assert.isNotNull(facet);
    });

    it('includes Error', () => {
      const facet = INP_BREAKDOWNS.find((b) => b.facetName === 'error');
      assert.isNotNull(facet);
    });

    it('includes 404', () => {
      const facet = INP_BREAKDOWNS.find((b) => b.facetName === 'four04');
      assert.isNotNull(facet);
    });

    it('includes Redirect', () => {
      const facet = INP_BREAKDOWNS.find((b) => b.facetName === 'redirect');
      assert.isNotNull(facet);
    });

    it('has matching col and facetName for all entries', () => {
      for (const bd of INP_BREAKDOWNS) {
        assert.strictEqual(bd.col, bd.facetName, `col should match facetName for ${bd.id}`);
      }
    });

    it('has unique IDs', () => {
      const ids = INP_BREAKDOWNS.map((b) => b.id);
