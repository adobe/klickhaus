 ...
 * Shared between loadLcpTimeSeries and loadLcpBreakdowns
...
/**
 * Fetch RUM data and render the LCP time series chart.
 * Uses viewType='lcp' for LCP-specific classification (good/ni/poor)
 * and applies interpolation for sparse CWV measurement buckets.
 * @param {object} requestContext - From startRequestContext()
 */
async function loadLcpTimeSeries(requestContext) {
  const { requestId, scope } = requestContext;
  const isCurrent = () => isRequestCurrent(requestId, scope);

  // Invalidate cache so concurrent loadLcpBreakdowns fetches fresh data
  cachedRumResult = null;
  ...
  try {
    ...
    // Cache result for loadLcpBreakdowns (if it hasn't fetched yet)
    cachedRumResult = result;

    // Apply interpolation to fill gaps where LCP measurements are sparse
...
/**
 * Render breakdown facet tables from RUM LCP data.
 * Uses the full facet set for comprehensive LCP analysis.
 * @param {object} requestContext - From startRequestContext()
 */
async function loadLcpBreakdowns(requestContext) {
  const { requestId, scope } = requestContext;
  const isCurrent = () => isRequestCurrent(requestId, scope);
...
// Initialize the dashboard with LCP-specific config
initDashboard({
  title: 'RUM LCP',
  ...
  loadTimeSeries: loadLcpTimeSeries,
  loadBreakdowns: loadLcpBreakdowns,
});
