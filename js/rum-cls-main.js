 ...
 * Shared between loadClsTimeSeries and loadClsBreakdowns
...
/**
 * Fetch RUM data and render the CLS time series chart.
 * Uses viewType='cls' for CLS-specific classification (good/ni/poor)
 * and applies interpolation for sparse CWV measurement buckets.
 * @param {object} requestContext - From startRequestContext()
 */
async function loadClsTimeSeries(requestContext) {
  const { requestId, scope } = requestContext;
  const isCurrent = () => isRequestCurrent(requestId, scope);

  // Invalidate cache so concurrent loadClsBreakdowns fetches fresh data
  cachedRumResult = null;
  ...
  try {
    ...
    // Cache result for loadClsBreakdowns (if it hasn't fetched yet)
    cachedRumResult = result;

    // Apply interpolation to fill gaps where CLS measurements are sparse
    const interpolatedChartData = interpolateCwvGaps(result.chartData);
...
/**
 * Render breakdown facet tables from RUM CLS data.
 * Uses the full facet set for comprehensive CLS analysis.
 * @param {object} requestContext - From startRequestContext()
 */
async function loadClsBreakdowns(requestContext) {
  const { requestId, scope } = requestContext;
  const isCurrent = () => isRequestCurrent(requestId, scope);
...
function setRumUrlState(creds) {
  ...
  renderRumNav(document.getElementById('rumNav'), 'cls');
}
...
// Initialize the dashboard with CLS-specific config
initDashboard({
  title: 'RUM CLS',
  ...
  loadTimeSeries: loadClsTimeSeries,
  loadBreakdowns: loadClsBreakdowns,
});
