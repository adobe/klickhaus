...
describe('detectStep', () => {
  ...
  it('should NOT highlight the 2xx spike at index 23 (21:27)', () => {
    ...
    if (result) {
      ...
      if (result.startIndex === 23 || result.endIndex === 23) {
        assert.notStrictEqual(result.category, 'success', 'Should not highlight 2xx spike - green spikes are good!');
  ...
  it('should ignore first 2 and last 2 data points', () => {
    ...
    if (result) {
      assert.ok(result.startIndex >= 2, 'Should not detect in first 2 points (incomplete bucket artifacts)');
      assert.ok(result.endIndex < realData.length - 2, 'Should not detect in last 2 points (data ingestion delay)');
    }
  });
...
describe('CDN operational requirements', () => {
  ...
  it('error spike beats green spike (10x weight)', () => {
    ...
    assert.ok(result, 'Should detect something');
    assert.strictEqual(result.category, 'error', 'Error spike (10x weight) should beat green spike (1x weight)');
  });
