...
/**
 * Reset the allowed columns cache. Called when state.breakdowns changes
 * (e.g., during tests or when switching data sources).
 */
export function resetAllowedColumnsCache() {
...
export function getAllowedColumns() {
  if (allowedColumnsCache) {
    return allowedColumnsCache;
  }
  const cols = new Set();
...
export function isFilterSuperset(current, cached) {
  for (const [sqlCol, cachedGroup] of Object.entries(cached || {})) {
    ...
    for (const entry of cachedGroup.includes || []) {
      if (!currentIncludes.has(entryKey(entry))) {
        return false;
      }
    }
    for (const entry of cachedGroup.excludes || []) {
      if (!currentExcludes.has(entryKey(entry))) {
        return false;
      }
    }
  }
